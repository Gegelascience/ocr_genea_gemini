...
function getRequestIA(keyword){
    if (keyword.length === 0){
        return `Extract the text. The text is in latin or in French or in Old French.` 

    } else {
        return `Extract paragraphs where text contains the keyword ${keyword}. The text is in latin or in French or in Old French.`
...
export class GeminiLogAnalyseClient {
    ...
    async analyseArchiveGemini(model,fileData, keyword) {
